...
import { PrismaClient, SellerType, PlateType, ListingStatus } from "@prisma/client";

const prisma = new PrismaClient();

interface PlateSeedItem {
  id: number;
  listingId: string;
  emirate: string;
  city: string;
  ...
  status: "active" | "pending" | "pending_verification" | "sold" | "rejected" | "archived";
  views: number;
  favorites: number;
  inquiries: number;
  createdAt: string;
...
function normalizeStatus(status: PlateSeedItem["status"]): ListingStatus {
...
function createSlug(item: PlateSeedItem): string {
...
async function ensureSeedUsers() {
  const passwordHash = await bcrypt.hash("SeedUser@123", 10);
...
async function main() {
  const datasetPath = path.resolve(__dirname, "../../uae_number_plates_500.json");
  const raw = await fs.readFile(datasetPath, "utf-8");
  const items = JSON.parse(raw) as PlateSeedItem[];

  const users = await ensureSeedUsers();

  let inserted = 0;
  for (const item of items) {
    const sellerType = item.sellerType as SellerType;
    const plateType = item.plateType as PlateType;
    const status = normalizeStatus(item.status);

    const userId =
      sellerType === "owner" ? users.owner : sellerType === "broker" ? users.broker : users.dealer;
